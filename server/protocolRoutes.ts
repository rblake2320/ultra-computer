...
export function registerProtocolRoutes(app: Express) {
  ...
  app.post("/api/protocols/code/interpret", async (req: Request, res: Response) => {
    ...
    try {
      const result = await cliToolEngine.executeCodeInterpreter(code, language as any);
      res.json(result);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/protocols/files/transform", async (req: Request, res: Response) => {
    ...
    try {
            const result = await cliToolEngine.executeFileTransform(inputPath, outputPath, transformType as any, options || {});
      res.json(result);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
